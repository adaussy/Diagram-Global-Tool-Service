...
/**
 * @author gdesq
 *  This Service allow other classes to get the types to fill in the popupbar
 */

public class PopupBarService {

	private final static PopupBarService service = new PopupBarService();

	public static PopupBarService getInstance() {
		return service;
	}
	...
	/**
	 * 
	 * Host is the container pointed by the mouse.
	 * @param host
	 * 
	 * @return
	 * Return a list of PopupBarType or emptyList if no type
	 */
	public List<PopupBarDescriptor> getTypesForPopupBar(IAdaptable host) {
		...
		if(editPart != null) {
			if(editPart.getRoot().getChildren().get(0) instanceof DiagramEditPart) {
				...
				// declaration
				List<PopupBarDescriptor> types = new ArrayList<>(1);
