...
/**
 * @author gdesq
 *         This Edit Policy is used to display handles to help user constructing links between objects of a diagram. The handles will display when
 *         pressing keys CTRL + MAJ.
 */
public class DgtsConnectionHandleEditPolicy extends GraphicalEditPolicy implements MouseMotionListener {
	...
	protected List<CustomConnectionHandle> getHandleFigures() {
		List<CustomConnectionHandle> list = new ArrayList<CustomConnectionHandle>(2);
	...
	protected void showHandles(Point referencePoint) {
		...
		if(shouldShowHandles()) {
			...
			for(Iterator<CustomConnectionHandle> iter = HandleRegistry.getInstance().getHandles().iterator(); iter.hasNext();) {
