...
/**
 * @author gdesq
 * This Edit Policy is used to create a a connection or a 
 * popup menu displaying which connections are available
 * when dragging a handle to a node part
 */
public class DgtsGraphicalNodeEditPolicy extends GraphicalEditPolicy {
	...
	protected class PromptAndCreateConnectionCommand extends CreateOrSelectElementCommand {
		...
		/**
		 * Cache the request because it needs to be passed to {@link #getCommandForMenuSelection(Object, CreateConnectionRequest)}.
		 */
		 ...
		 */
		public PromptAndCreateConnectionCommand(List<IElementType> content, CreateConnectionRequest request) {
	...
	protected Command getUnspecifiedConnectionCompleteCommand(DgtsCreateUnspecifiedTypeConnectionRequest request) {
		...
		List<IElementType> menuContent = getConnectionMenuContent(request);
	 ...
	 */
	protected ICommand getPromptAndCreateConnectionCommand(List<IElementType> content, CreateConnectionRequest request) {
	 ...
	 *        the connection type as specified in {@link #getConnectionMenuContent(CreateConnectionRequest)}
	 * @param request
	 ...
	 * connector content if the given request is a <code>CreateUnspecifiedTypeConnectionRequest</code> using the types it
	 * holds or the types retrieved from the Modeling Assistant Service.
	 * 
	 * <p>
	 * If a subclass wishes to provide additional element types they should consider providing these in a Modeling Assistant Provider. If a subclass
	 * wishes to provide connector choices that are not elements types they may provide them here, in this case, the label provider for
	 ...
	 */
	protected List<IElementType> getConnectionMenuContent(CreateConnectionRequest request) {
