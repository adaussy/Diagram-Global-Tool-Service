...
public class CustomPromptForConnectionAndEndCommand extends
		PromptForConnectionAndEndCommand {

	public CustomPromptForConnectionAndEndCommand(
			CreateConnectionRequest request, IGraphicalEditPart containerEP) {
	...
	protected List getConnectionMenuContent() {
		...
		if (request instanceof CreateUnspecifiedTypeConnectionRequest) {
			...
			if (((CreateUnspecifiedTypeConnectionRequest) request)
				...
				allRelTypes = isDirectionReversed() ? CustomModelingAssistantService
					.getInstance().getRelTypesOnTarget(getKnownEnd())
					: CustomModelingAssistantService.getInstance()
	...
	protected List getEndMenuContent(Object connectionItem) {
		if (connectionItem instanceof IElementType) {
			...
			List menuContent = isDirectionReversed() ? CustomModelingAssistantService
				.getInstance().getTypesForSource(getKnownEnd(), connectionType)
				: CustomModelingAssistantService.getInstance().getTypesForTarget(
		...
		if (result instanceof List) {
			...
			if (resultList.size() == 2) {
				...
				if (targetResult.equals(EXISTING_ELEMENT)) {
					targetResult = isDirectionReversed() ? CustomModelingAssistantService
	...
	private boolean supportsExistingElement(IElementType connectionType) {
		if (isDirectionReversed()) {
			if (CustomModelingAssistantService.getInstance().provides(
				new SelectExistingElementForSourceOperation(getKnownEnd(),
					connectionType))) {
				return true;
			}
		} else if (CustomModelingAssistantService.getInstance().provides(
